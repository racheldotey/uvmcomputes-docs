...
def Visualize_Data(content):
    ...
    resultText = "Individual #{} - Age {}, Origin {}, Fitness {}\n\r"
    #resultElem = document.getElementById ('result')
    print(resultText.format(content['id'], content['age'], content['origin'], content['fitness']))
...
class AFPO():
    ...
    def __init__(self, parameters=dict()):
        ...
        # __dict__ not supported by Transcrypt 
        # https://github.com/QQuick/Transcrypt/issues/397
        # self.__dict__.update(AFPO.P, **parameters)

        self.popsize = parameters['popsize']
        self.max_gene_len = parameters['max_gene_len']
        self.target = parameters['target']
        self.max_generations = parameters['max_generations']

        self.save_results = list()
    ...
    def Evolve(self):
        ...
        for g in range(self.max_generations):
            for child in self.children:
                ...
                resultText = "Individual #{} - Age {}, Origin {}, Fitness {}\n\r"
                self.save_results.append(resultText.format(child['id'], child['age'], child['origin'], child['fitness']))

                # Print to Console
                self.save_results.append(child)           

            self.Selection()
            
            # Report current champion fitness:
            champ = self.champion['fitness']
            self.save_results.append("Generation {0}/{1}: Champion={2}".format(self.current_gen, self.max_generations, champ))
    ...
    def Selection(self):
        ...
        if self.current_gen == 0:
            ...
            self.Mature()
            self.Find_Champion(self.champion['fitness'])        
